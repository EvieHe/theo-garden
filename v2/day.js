...
function img(src,cls='',date=''){return `<figure class="story-photo ${cls}"><img src="${assetUrl(src)}" alt=""><figcaption>${esc(date.replaceAll('-',' · '))}</figcaption></figure>`}
...
function renderStory(entries,date){
 ...
 else if(mode==='solo')flow.innerHTML=`<section class="solo-layout story-reveal">${img(a,'is-hero',date)}<div class="story-copy"><small>ONE FRAME</small><h2>This day, kept.</h2><div class="real-copy">${lead}</div></div></section>`;
 else if(mode==='duet')flow.innerHTML=`<section class="duet-layout story-reveal">${img(a,'is-large',date)}${img(b,'is-small',date)}<div class="story-copy"><small>TWO FRAMES</small><h2>The same day, twice remembered.</h2><div class="real-copy">${lead}</div></div></section>`;
 else if(mode==='trio')flow.innerHTML=`<section class="trio-layout story-reveal">${img(a,'is-large',date)}${img(b,'is-tall',date)}${img(c,'is-small',date)}<div class="story-copy"><small>THREE FRAMES</small><h2>A little more to remember.</h2><div class="real-copy">${lead}</div></div></section>`;
 else flow.innerHTML=`<section class="opening-spread story-reveal"><div class="story-copy story-copy--lead"><small>THIS DAY</small><h2>A day worth keeping.</h2><div class="real-copy">${lead}</div></div>${img(a,'is-wide',date)}</section><section class="offset-pair story-reveal">${img(b,'is-tall',date)}${img(c,'is-small',date)}<div class="story-copy"><small>MORE FRAMES</small><h2>Small pieces of the same day.</h2></div></section>${d?`<section class="full-bleed-memory story-reveal">${img(d,'is-cinema',date)}</section>`:''}${e?`<section class="closing-pair story-reveal"><div class="story-copy"><small>ONE LAST FRAME</small><h2>Ordinary is enough.</h2></div>${img(e,'is-polaroid',date)}</section>`:''}${rest.length?`<section class="film-strip story-reveal">${rest.map(x=>img(x,'',date)).join('')}</section>`:''}`;
 observe();
}
